         ...
         ProduitDAO pd=null;
            ...
            pd = new ProduitDAO();
              ...
              int qte_stock=Integer.parseInt(request.getParameter("qte_stock"));
                ...
                try {
           pd.modifierProduit(model,id_categorie,lib,prix);
           pd.modifierQteProduit(model, qte_stock);
       this.getServletContext().getRequestDispatcher("/gestion_produits.jsp?produit_modifie").forward(request, response);
        } catch (SQLException ex) {
        } catch (ClassNotFoundException ex) {
        }
    }
